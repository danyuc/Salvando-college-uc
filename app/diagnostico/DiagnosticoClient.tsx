...
import { generateQuestion } from "@/lib/precalculo-engine"
import { getMat1000DiagnosticSubtemas, normalizeMat1000Evaluation } from "@/lib/mat1000-diagnostic-scope"
...
export default function DiagnosticoPage() {
  ...
  const evaluation = normalizeMat1000Evaluation(searchParams.get("evaluation") || "I1")

  const questions = useMemo(() => {
    const subtemas = getMat1000DiagnosticSubtemas(evaluation)

    return subtemas.map((subtema, index) =>
      generateQuestion({
        subtema,
        dificultad: index < 4 ? "baja" : index < 8 ? "media" : "alta",
        origen: "prueba_real",
      })
    )
  }, [evaluation])
  ...
  return (
    <main style={{
      ...
      <section style={{
        ...
        <p style={{ color: "#93c5fd", fontWeight: 900 }}>MAT1000 · Diagnóstico real · {evaluation}</p>
