...
import type { CrshSensor } from "@/lib/cardenal-respira"
  ...
  return (
    <article className="rounded-[2rem] border border-white/10 bg-white/[0.07] p-5 text-white shadow-2xl backdrop-blur-xl">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          <p className="text-xs font-black uppercase tracking-[0.22em] text-cyan-200">{sensor.status}</p>
          ...
          <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-300">{sensor.description}</p>
        </div>
        <a
      ...
      <div className="mt-4 rounded-2xl border border-cyan-300/20 bg-cyan-300/10 p-4 text-sm font-semibold leading-6 text-cyan-50">
        Si el dashboard bloquea la previsualización por seguridad, usa el botón “Abrir dashboard” para verlo en una nueva pestaña.
      </div>
    </article>
  )
}
