...
const initialForm: FormState = { subject: 'PSI1101', title: '', type: 'prueba', grade: '', weight: '' }
...
function gradeValue(ev: Evaluation) {
  return Number(ev.grade ?? 0)
}
...
export default function NotesView() {
  ...
  const globalAverage = useMemo(() => {
    const valid = analysis.map((i) => i.projectedFinal ?? i.currentAverage).filter((n): n is number => typeof n === 'number')
  ...
  async function createEv(e: FormEvent) {
    ...
    const grade = safeGrade(form.grade)
  ...
  const handlePerusall = useCallback(async (grade: number) => {
    ...
    const existing = evaluations.find((ev) => ev.subject === 'PSI1101' && String(ev.title ?? '').toLowerCase() === 'perusall acumulado')

    if (existing?.id) {
      const current = Number(existing.grade ?? 0)
      if (Math.abs(current - cleanGrade) < 0.01) return
      await updateEvaluation(existing.id, { grade: cleanGrade, weight_percent: 30 } as any)
    } else {
      await createEvaluation({
        user_id: userId,
        subject: 'PSI1101',
  ...
  if (loading) {
    return <main style={styles.container}><section style={styles.panel}>Cargando notas inteligentes...</section></main>
  }
    ...
    <main style={styles.container}>
      <section style={styles.hero}>
        <div>
          ...
          <p style={styles.subtitle}>Cada ramo tiene un color fijo para conectar notas, diagnósticos y prácticas.</p>
        </div>
        <div style={styles.heroStats}>
          <div style={styles.heroNumber}>{globalAverage === null ? '—' : globalAverage.toFixed(2)}</div>
      ...
      <section style={styles.topGrid}>
        <form style={styles.panel} onSubmit={createEv}>
          ...
          <div style={styles.formGrid}>
            <select style={styles.input} value={form.subject} onChange={(e) => updateForm('subject', e.target.value)}>
              {subjects.map((subject) => <option key={subject.code} value={subject.code}>{subject.name} · {subject.code}</option>)}
            </select>
            ...
            <select style={styles.input} value={form.type} onChange={(e) => updateForm('type', e.target.value)}>
              ...
              <option value="interrogacion">Interrogación</option>
              <option value="taller-fuentes">Taller de fuentes</option>
              <option value="evaluacion-oral">Evaluación oral</option>
              <option value="examen">Examen</option>
            </select>
            <input style={styles.input} type="number" min={1} max={7} step={0.1} placeholder="Nota" value={form.grade} onChange={(e) => updateForm('grade', e.target.value)} />
        ...
        <aside style={styles.panel}>
          ...
          {alerts.length === 0 ? <p style={styles.muted}>Todo está bajo control por ahora.</p> : (
      ...
      {analysis.length === 0 && <section style={styles.emptyState}>Aún no hay notas registradas.</section>}
      ...
      <section style={styles.cardsGrid}>
        {analysis.map((item) => {
          ...
          return (
            <article key={item.subject} style={{ ...styles.subjectCard, borderTop: `5px solid ${color}`, boxShadow: `0 0 0 1px ${color}33` }}>
              <header style={styles.cardHeader}>
                <div>
                  <div style={{ ...styles.subjectPill, background: `${color}22`, color, border: `1px solid ${color}55` }}>{item.subject}</div>
                  <h2 style={styles.subjectTitle}>{getSubjectName(item.subject)}</h2>
                  <p style={styles.muted}>{item.completedWeight.toFixed(0)}% rendido · {item.remainingWeight.toFixed(0)}% restante</p>
                </div>
                <div style={gradeCircle(item.risk, color)}>{item.currentAverage === null ? '—' : item.currentAverage.toFixed(2)}</div>
              </header>
              ...
              <div style={styles.progressTrack}><div style={{ ...styles.progressFill, width: `${Math.min(item.completedWeight, 100)}%`, background: color }} /></div>

              {isPsychology(item.subject) && <div style={{ ...styles.perusallBox, border: `1px solid ${color}55`, background: `${color}12` }}><PerusallTracker onGradeChange={handlePerusall} /></div>}

              <div style={styles.box}><strong>Lectura IA</strong><p style={styles.muted}>{item.recommendation}</p></div>
...
function Metric({ label, value }: { label: string; value: string }) {
  return <div style={styles.metricCard}><span style={styles.metricLabel}>{label}</span><strong style={styles.metricValue}>{value}</strong></div>
}
...
function gradeCircle(risk: string, color: string): CSSProperties {
  let border = color
  if (risk === 'critico' || risk === 'alto') border = '#ef4444'
  if (risk === 'medio') border = '#f59e0b'
  return { width: 76, height: 76, borderRadius: 999, display: 'grid', placeItems: 'center', border: `2px solid ${border}`, background: `${border}18`, fontSize: 23, fontWeight: 950 }
}

const styles: Record<string, CSSProperties> = {
  container: { padding: 24, display: 'grid', gap: 20, color: 'white' },
  hero: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 20, padding: 24, borderRadius: 28, background: 'linear-gradient(135deg, rgba(37,99,235,0.35), rgba(2,6,23,0.92))', border: '1px solid rgba(255,255,255,0.12)' },
  ...
  panel: { padding: 18, borderRadius: 24, background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.10)' },
  ...
  input: { width: '100%', padding: '12px 13px', borderRadius: 14, border: '1px solid rgba(255,255,255,0.12)', background: 'rgba(15,23,42,0.88)', color: 'white', outline: 'none' },
  primaryButton: { width: '100%', marginTop: 12, padding: '13px 16px', borderRadius: 14, border: 'none', background: '#2563eb', color: 'white', fontWeight: 900, cursor: 'pointer' },
  ...
  subjectPill: { width: 'fit-content', padding: '5px 9px', borderRadius: 999, fontSize: 12, fontWeight: 900, marginBottom: 8 },
  ...
  perusallBox: { marginTop: 14, padding: 13, borderRadius: 18 },
