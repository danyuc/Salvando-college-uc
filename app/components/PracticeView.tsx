...
const PrecalculoVisual = dynamic(() => import('./PrecalculoVisual'), { ssr: false })
  ...
  return (
    <main style={container}>
      ...
      {diagnosticRequired && (
        ...
        {selectedSubject === "MAT1000" &&
          Array.isArray((currentQuestion as any)?.pasos) && (
            <PrecalculoSteps
              pasos={(currentQuestion as any).pasos}
              animaciones={(currentQuestion as any).animaciones || []}
            />
          )}
`
              )
...
<span>Cantidad</span>
      ...
      {currentQuestion && !finished && (
        <section style={{ ...questionCard, border: `2px solid ${subjectMeta.color}`, boxShadow: `0 0 42px ${subjectMeta.color}66`, background: `linear-gradient(135deg, ${subjectMeta.color}22, rgba(2,6,23,0.94))` }}>
          ...
          <h2 style={questionTitle}>{currentQuestion.pregunta}

        {selectedSubject === "MAT1000" &&
          Array.isArray((currentQuestion as any)?.visualizacion?.parametros?.puntos) && (
            <PrecalculoVisual
